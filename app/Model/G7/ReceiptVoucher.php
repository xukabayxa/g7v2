...
// Đối tượng nhận phí
    ...
    public static function searchByFilter($request)
    {
        ...
        $result = $result->where('g7_id', $user->g7_id);
        ...
        if ($request->object) {
            $result = $result->where('payer_type_id', $request->object);
        }

        if ($request->payer_name) {
            $result = $result->whereHasMorph('payer', [User::class, Customer::class, Supplier::class], function ($q) use ($request) {
                $q->where('payer_id', $request->payer_name);
            });
        }
