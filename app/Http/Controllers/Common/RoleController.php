...
class RoleController extends Controller
{
	...
	public function store(Request $request)
	{
		...
		try {
			...
			if (Auth::user()->type == User::SUPER_ADMIN) {
				$object->type = User::UPTEK;
			} else if (Auth::user()->type == User::UPTEK) {
				$object->type = User::G7;
			} else {
				$object->type = User::NHAN_VIEN_G7;
			}
	...
	public function update(Request $request, $id)
	{
		...
		if (!Auth::user()->g7_id) {
			$rule = array_merge($rule, [
				'display_name' => 'required|unique:roles,name,'.$id.'|unique:roles,display_name,'.$id,
			]);
		}
