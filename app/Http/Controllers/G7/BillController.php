...
class BillController extends Controller
{
	...
	public function store(Request $request)
	{
		...
		try {
			...
			if($request->allow_point == 1) {
				// Thanh toán điểm thưởng
				if($customer->current_point < $request->point_pay) {
					$json->success = false;
					$json->errors = $validate->errors();
					...
					$point_money = $request->point_pay * AccumulatePoint::getPointRate();
					$object->payed_value = $point_money;

					$object->allow_point = $request->allow_point;
					$object->point_pay = $request->point_pay;
					$object->point_money = $point_money;
					if ($object->status == ThisModel::DA_DUYET) {
						// Lưu số tiền đã thanh toán bằng điểm
						$object->payed_value = $point_money;
					}
			...
			if ($object->status == ThisModel::DA_DUYET) {
				...
				$customer->current_point = $customer->current_point - $request->point_pay;
				$customer->save();
				// Tính lại điểm
	...
	public function update(Request $request, $id)
	{
		...
		try {
			...
			if(AccumulatePoint::where('id',1)->first()->type == 1) {
				...
				if($request->list_services && $request->list_products) {
					$points = $request->service_total_point + $request->product_total_point;
				}
			}
    ...
    public function getDataForShow2($id)
    {
        $json = new stdClass();
