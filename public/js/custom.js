...
function sendRequest(options = {}, scope = null) {
    let loading = $(
        `<div class = '_loaderContainer'><div class='_loaderOverlay'></div><div class ='_loaderContent'><img src = '/img/icons/g7_logo.png' class = 'logo'><div class='_loader'><span></span><span></span><span></span><span></span><span></span></div></div></div>`
    );
    $("body").append(loading);
    $.ajax({
        headers: {
...
function getRowData(elm, datatable) {
    let current_row = $(elm).parents('tr');
    if (current_row.hasClass('child')) {
        current_row = current_row.prev();
    }
    console.log(current_row)
    return datatable.row(current_row).data();
}
